...
contract Pool is CoreV2 {
     ...
     */
    constructor(uint256 ampFactor_, uint256 haircutRate_) public {
    ...
    function setMasterWombat(address masterWombat_) external {
     ...
     */
    function setAmpFactor(uint256 ampFactor_) external {
    ...
    function setFee(uint256 lpDividendRatio_, uint256 retentionRatio_) external {
    ...
    function deposit(
        ...
        if (!shouldStake) {
            ...
            uint256 pid = MasterWombatV2(masterWombat).getAssetPid(address(asset));
            MasterWombatV2(masterWombat).depositFor(pid, liquidity, to);
        }
