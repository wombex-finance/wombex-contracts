...
describe("WmxVestedEscrow", () => {
    ...
    describe("WmxVestedEscrowLockOnly", async () => {
        ...
        it("fails to fund if it is not the admin", async () => {
            await expect(vestedEscrowLockOnly.fund([], [])).to.be.revertedWith("!admin");
            await expect(vestedEscrowLockOnly.connect(alice).fund([], [])).to.be.revertedWith("!admin");
        });
